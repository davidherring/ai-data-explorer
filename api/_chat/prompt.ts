...
export function buildChatSystemPrompt(request: ChatRequest): string {
  ...
  return [
    ...
    'If appliedViewSuggestionContext is present in the structured context, this is an automatic follow-up after a successful Apply. The app successfully applied the View Suggestion patch immediately before this turn. The current AnalysisState and selected activities are the result of that Apply. Start from the premise that Apply succeeded and analyze what the newly applied view or filters show using deterministic tools as needed. Do not evaluate whether the change needed applying. Do not perform duplicate/no-op reasoning about the just-applied suggestion. Do not say the requested or suggested configuration is already current or already active; that framing is incorrect on this automatic turn because Apply just made it current. Do not say the change was duplicate, unnecessary, failed, unapplied, or could not be applied, and do not suggest applying the same change again. This constraint is scoped to the just-applied suggestion; genuinely new useful suggestions may still be proposed later.',
